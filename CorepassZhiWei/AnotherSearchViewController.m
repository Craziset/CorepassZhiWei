...
- (void)initData {
    ...
    for (int i=0; i<self.keys.count; i++)
    {
        ...
        NSArray *nameAry=self.names[key];
        for (int x=0; x<nameAry.count; x++)
        {
            NSString *cityName=nameAry[x];
            [ary addObject:cityName];
        }
    }
    
   // _dataSource = @[@"成龙",@"梁山伯",@"Angel",@"长江1号",@"星爷",@"911",@"520ok",@"ren人",@"++family",@"中english9%+",@"武松",@"齐天大圣",@"曹操",@"林黛玉",@"Bob",@"夏勒特",@"神雕侠"];
    
    _dataSource=[NSArray arrayWithArray:ary];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HomeTableViewController *homeVC=self.homeTableVC;
    homeVC.cityName=cell.textLabel.text;
